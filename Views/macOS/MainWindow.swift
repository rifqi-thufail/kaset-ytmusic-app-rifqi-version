...
struct MainWindow: View {
    ...
    /// Pending navigation from fullscreen player
    @State private var pendingArtistNavigation: Artist?
    @State private var pendingAlbumNavigation: Album?
    ...
    var body: some View {
        ...
        .overlay {
            ...
            // Full-screen player overlay - sweeps up from the player bar
            if self.playerService.showFullScreenPlayer, let client = ytMusicClient {
        ...
        .animation(.spring(response: 0.5, dampingFraction: 0.85), value: self.playerService.showFullScreenPlayer)
        .onChange(of: self.pendingArtistNavigation) { _, artist in
            if let artist {
                // Navigate to search view which can show artist
                self.navigationSelection = .search
                self.pendingArtistNavigation = nil
            }
        }
        .onChange(of: self.pendingAlbumNavigation) { _, album in
            if let album {
                // Navigate to search view which can show album
                self.navigationSelection = .search
                self.pendingAlbumNavigation = nil
            }
        }
    ...
    private var mainContent: some View {
        if let client = ytMusicClient {
            ...
            .toolbar(self.playerService.showFullScreenPlayer ? .hidden : .automatic, for: .windowToolbar)
            .toolbar {
                ToolbarItem(placement: .primaryAction) {
                    ...
                    .help("Search (⌘K)")
                    .accessibilityIdentifier(AccessibilityID.MainWindow.searchButton)
                }
            }
        } else {
            self.loadingView
