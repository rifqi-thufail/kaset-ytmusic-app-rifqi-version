...
enum SearchResponseParser {
    ...
    private static func parseSearchResultItem(_ data: [String: Any]) -> SearchResultItem? {
        ...
        // Try multiple methods to get videoId for songs
        var videoId: String?

        // Method 1: playlistItemData
        if let playlistItemData = responsiveRenderer["playlistItemData"] as? [String: Any],
           let vid = playlistItemData["videoId"] as? String
        {
            videoId = vid
        }

        // Method 2: Use ParsingHelpers.extractVideoId (checks overlay and navigationEndpoint)
        if videoId == nil {
            videoId = ParsingHelpers.extractVideoId(from: responsiveRenderer)
        }

        // If we found a videoId, this is a song
        if let videoId {
            return self.parseSongFromResponsiveRenderer(responsiveRenderer, videoId: videoId)
        }

        // Check navigation endpoint for other types (albums, artists, playlists)
