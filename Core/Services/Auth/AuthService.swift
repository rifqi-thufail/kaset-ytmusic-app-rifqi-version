...
final class AuthService: AuthServiceProtocol {
    ...
    /// Clears existing session and immediately starts login flow,
    /// staying in loggingIn state to show the login sheet.
    func switchAccount() async {
        self.logger.info("Switching Google account")

        // Clear all WebKit data (cookies, cache, etc.)
        await self.webKitManager.clearAllData()

        // Immediately transition to logging in (not logged out)
        // This keeps showing the login sheet instead of onboarding
        self.state = .loggingIn
